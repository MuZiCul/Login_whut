...
class NetConnect:
    ...
    def check_connect(self):
        print('开始检查网络连接！')
        try:
            response1 = requests.get(url=self.host, headers=self.headers, timeout=1,
                                    proxies={'http': f'http://{self.get_server_form_Win()}',
                                             'https': f'http://{self.get_server_form_Win()}'})
            if 199 < response1.status_code < 300:
                print('网络已连接！')
                return True
        except Exception as e:
            print('校园网未登录！')
        try:
            response2 = requests.get(url=self.host, headers=self.headers, timeout=1)
            if 199 < response2.status_code < 300:
                print('网络已连接！')
                return True
        except Exception as e:
            print('校园网未登录！')
    ...
    def connect_whut(self):
        print('开始联网！')
        # 注：上游此处硬编码了真实账号与密码，本快照已脱敏为占位符
        username = 'your-account'
        password = '{B}<base64-encoded-password>'
